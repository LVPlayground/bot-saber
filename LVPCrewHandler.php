...
class LVPCrewHandler implements LVPCommandRegistrar {
	...
	public function checkLevel($sNickname, $nLevel)
	{
		switch ($nLevel) {
			case LVP::LEVEL_MANAGEMENT:
				$aCrew = &$this->m_aManagement;
				break;

			case LVP::LEVEL_ADMINISTRATOR:
				$aCrew = &$this->m_aAdministrator;
				break;

			case LVP::LEVEL_DEVELOPER:
				$aCrew = &$this->m_aDeveloper;
				break;
		}
	...
	public function removeAdmin($sNickname, $bFromCommand = false)
	{
		...
		if ($bFromCommand) {
			...
			$this->IrcService->info(null, LVP::CREW_CHANNEL, $sNickname . ' is no longer an administrator.');
		}
	}
	...
	public function addModLogin($sFakeName, $sRealName, $nLevel)
	{
		...
		$this->IrcService->info(null, LVP::CREW_CHANNEL, $sMessage . '.');

		$pPlayer = $this->PlayerService->getPlayerByName($sFakeName);
		if ($pPlayer != null) {
	...
	public function removeModLogin($sFakeName, $bSilent = false)
	{
		...
		if (!$bSilent) {
			...
			$this->IrcService->info(null, LVP::CREW_CHANNEL, ModuleBase::BOLD . $sFakeName .
	...
	public function addTempAdmin($sTempAdminName, $sCrewMember)
	{
		...
		$pPlayer = $this->PlayerService->getPlayerByName($sTempAdminName);
		if ($pPlayer != null) {
	...
	private function handleIngameCrew($nLevel)
	{
		if ($nLevel >= LVP::LEVEL_ADMINISTRATOR) {
			...
			if (count($this->m_aTempAdmin) == 0) {
				...
				foreach ($this->m_aTempAdmin as $sNickname => $sCrewMember) {
			...
			if (count($this->m_aModLogin) == 0) {
				...
				foreach ($this->m_aModLogin as $sNickname => $sRealname) {
		...
		// Permanent crew.
		$aIngameCrew = array();
		foreach ($this->PlayerService as $pPlayer) {
			if ($pPlayer['Level'] >= LVP::LEVEL_ADMINISTRATOR) {
				$aIngameCrew[] = $pPlayer;
			}
		}

		$nPlayers = count($this->PlayerService);
	...
	public function syncPlayers()
	{
		...
		foreach ($tempCrew as $sNickname => $sCrewMember) {
			if ($playerService->getPlayerByName($sNickname) === null) {
