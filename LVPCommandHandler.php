...
class LVPCommandHandler extends LVPEchoHandlerClass {
	...
	public function register(LVPCommand $pCommand) {
		...
		$aCommands[$pCommand->getTrigger()] = $pCommand;
	}
	...
	public function unregister(LVPCommand $pCommand) {
		...
		if (isset($aCommands[$pCommand->getTrigger()])) {
			unset($aCommands[$pCommand->getTrigger()]);
		}
	}
	...
	public function processOutput($pBot, $sChannel, $sOutput, $nMode = LVPCommand::MODE_IRC) {
		...
		if ($pBot == null) {
			$pBot = $this->IrcService->getBot($sChannel);
		}
		...
		foreach (explode("\n", $sOutput) as $sLine) {
			...
			switch ($nReturnCode) {
				default:
				case LVPCommand::OUTPUT_NORMAL:
					$sCallback = 'privmsg';
					break;

				case LVPCommand::OUTPUT_ERROR:
					$sCallback = 'error';
					break;

				case LVPCommand::OUTPUT_INFO:
					$sCallback = 'info';
					break;

				case LVPCommand::OUTPUT_NOTICE:
					$sCallback = 'notice';
					break;

				case LVPCommand::OUTPUT_SUCCESS:
					$sCallback = 'success';
					break;

				case LVPCommand::OUTPUT_USAGE:
					$sCallback = 'usage';
					break;
			}

			call_user_func(array($this->IrcService, $sCallback), $pBot, $sChannel, $sLine, $nMode);
		}
	}
}
